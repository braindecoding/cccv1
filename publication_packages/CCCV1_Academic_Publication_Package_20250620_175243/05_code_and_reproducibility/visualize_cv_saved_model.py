    ...
    if args.dataset == 'all':
        ...
        for dataset_name in datasets:
            ...
            if args.save:
                ...
                # Save PNG
                fig_path_png = save_dir / f"cv_model_reconstruction_{dataset_name}.png"
                fig.savefig(fig_path_png, dpi=300, bbox_inches='tight')
                print(f"💾 Saved PNG: {fig_path_png}")

                # Save SVG
                fig_path_svg = save_dir / f"cv_model_reconstruction_{dataset_name}.svg"
        ...
        if len(successful_datasets) > 1:
            ...
            if args.save:
                # Save PNG
                summary_path_png = save_dir / "cv_model_summary_all_datasets.png"
                summary_fig.savefig(summary_path_png, dpi=300, bbox_inches='tight')
                print(f"💾 Summary PNG saved: {summary_path_png}")

                # Save SVG
                summary_path_svg = save_dir / "cv_model_summary_all_datasets.svg"
                summary_fig.savefig(summary_path_svg, format='svg', bbox_inches='tight')
                print(f"💾 Summary SVG saved: {summary_path_svg}")
        ...
        if args.save:
            ...
            # Save PNG
            fig_path_png = save_dir / f"cv_model_reconstruction_{args.dataset}.png"
            fig.savefig(fig_path_png, dpi=300, bbox_inches='tight')
            print(f"💾 Saved PNG: {fig_path_png}")

            # Save SVG
            fig_path_svg = save_dir / f"cv_model_reconstruction_{args.dataset}.svg"
            fig.savefig(fig_path_svg, format='svg', bbox_inches='tight')
            print(f"💾 Saved SVG: {fig_path_svg}")
