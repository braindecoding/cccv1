"""
Mind-Vis Training Script
=======================

Train Mind-Vis following exact methodology from CVPR 2023 paper.
...
def train_mind_vis(dataset_name: str, device='cuda', epochs=100, 
                  batch_size=16, lr=1e-3, save_model=True):
    """Train Mind-Vis model on specified dataset"""
    
    print(f"🧠 MIND-VIS TRAINING")
...
def main():
    parser = argparse.ArgumentParser(description='Train Mind-Vis Model')
    ...
    if args.dataset == 'all':
        ...
        for dataset in datasets:
            ...
            try:
                model, best_loss, history = train_mind_vis(
        ...
        print(f"\n{'='*80}")
        print("MIND-VIS TRAINING SUMMARY")
        print(f"{'='*80}")
