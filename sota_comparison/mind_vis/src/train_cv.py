"""
Mind-Vis CV-Compliant Training Script
====================================

Train Mind-Vis using unified CV framework for academic-compliant comparison.
Maintains exact methodology from CVPR 2023 paper while ensuring fair comparison.
...
def train_mind_vis_fold(train_loader, val_loader, input_dim, device='cuda', 
                       fold=0, epochs=100, lr=0.001, patience=15, **kwargs):
    """
    Train Mind-Vis for one CV fold
    ...
    Returns:
        Trained model
    """
    # Create Mind-Vis model
    model = MindVisModel(
        fmri_input_dim=input_dim,
        device=device
    ).to(device)
...
def evaluate_mind_vis_fold(model, val_loader, device='cuda'):
    """
    Evaluate Mind-Vis for one CV fold
    
    Args:
        model: Trained Mind-Vis model
        val_loader: Validation data loader
        device: Computing device
...
def train_mind_vis_cv(dataset_name, device='cuda', n_folds=10):
    ...
    results = cv_framework.evaluate_method_cv(
        method_name=f"Mind-Vis-{dataset_name}",
        train_func=train_mind_vis_fold,
        evaluate_func=evaluate_mind_vis_fold,
        X_train=X_train,
        y_train=y_train,
        X_test=X_test,
        y_test=y_test,
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train Mind-Vis with Unified CV Framework')
    ...
    print("🧠 Mind-Vis CV-Compliant Training")
    print("=" * 40)
